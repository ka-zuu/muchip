...
| **Player** | 曲名・ゲーム名・作者・著作権・トラック `n/N`・経過/全体時間・シークバー・**現在のファイルが属するディレクトリのファイル一覧（中央。Browserと同じ一覧で、再生中のファイルをハイライト）**・波形ビジュアライザ（下部） |
...
| ボタン | Browser | Player |
|---|---|---|
| D-Pad Up/Down | カーソル移動 | 前/次の`.gbs`ファイルへ切替（即座に再生開始。中央のファイル一覧のカーソルも連動） |
