...
- **`text_dup_utf8()`**: 判定順「①全バイトASCII→そのまま ②妥当なUTF-8
  ...
  ③のCP932判定は、NSF/GBSのゲーム名等が固定32バイトであるため日本語の
  2バイト目がフィールド境界で切り捨てられた実ファイルが存在する問題
  （Issue #40。実機の`Super Chinese 2`等で確認）へ、末尾が2バイト文字の
  リードバイト1個だけで切れている場合に限り例外を認めている:
  その1バイトを黙って落とし、手前までを変換する。緩和はCP932側だけに
  留め、②のUTF-8判定は厳密なまま変更していない（②を③より先に試す
  不変条件を壊すと日本語UTF-8の誤判定を招くため）。末尾以外での破綻は
  従来通り④のフォールバックに落ちる。
