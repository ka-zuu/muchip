# 【凍結アーカイブ】旧 PLAN.md（P0〜P13 / Issue #2〜#29、v1.0.0〜v1.7.0）

この文書は 2026-08（Issue #30 の `.md` 整理）で凍結した。**追記しないこと。**
いま有効な設計判断は [`../design-notes.md`](../design-notes.md) に、
日々のコーディング規約・検証手順は [`../../CLAUDE.md`](../../CLAUDE.md) にある。
ここに残っているのは当時の作業計画・調査経緯・実機検証の実施記録で、
現在の実装と食い違う記述を含む（例: 冒頭の「muGBS」「P0〜P8完了」)。
本文中の相対リンク（`./SPEC.md` 等）も凍結時点のパスのままで、
このファイルの新しい場所（`docs/history/`）からは正しく辿れない。

---

# PLAN.md — muGBS 実装計画（凍結時点の原文）
...
5. **パッケージング・GUI**: `./scripts/package.sh`で`.muxapp`を作成
   ...
   - Player: `.nsf`を開いて`PLAYING`状態になり、波形ビジュアライザが
     ...
     既知の仕様どおりで、今回の不具合ではない。この制限自体は後に
     Issue #29（v1.7.0）で解消した）
...
表示だけでもmuChip側で直せないか、という依頼だった。

### 原因調査

添付されたNSFのヘッダを直接バイト単位でダンプして確認した。
`0x0E`（曲名, 32B）は `83 5F 83 45 83 93 ...`、`0x4E`（著作権, 32B）は
`... 83 65 83 4E 83 6D ...` で、いずれも**CP932(Shift_JIS)として解釈すると
正しい日本語になる**（曲名「ダウンタウンスペシャル くにおく」、著作権
「1991/07/26 テクノスジャパン」）。NESM(NSF)仕様ではこれらのフィールドは
ASCII前提なので、SJISを書き込むこと自体が非準拠であり、ユーザーの見立て
どおりファイル側の問題だった。抽出元と見られる「ファミコン音源」
(http://mrchecker.web.fc2.com/music/famicom_ongaku.html) のページも
確認したが、出力文字コードを選ぶ設定項目は無く、内蔵のタイトルDBが
SJISのまま埋め込まれる作りに見えたため、**元ファイルを直すツールは
作らない**（ユーザー確認済み）方針にした。

muChip側でも二重に化けていた:

1. `src/ui.c`の`utf8_next()`がUTF-8前提でSJISバイト列を誤読する
2. `vendor/font8x8`がASCII(U+0000-007F)のみで日本語グリフを持たない
   （PLAN.md 2291行目付近、Issue #2の実機検証時点で既知の制限として
   記録済みだった）

### 対応方針（ユーザー確認済み）

1. 日本語グリフは**JIS第1・第2水準フル**（美咲フォント8x8を同梱）。
   「かな・記号のみ」「変換だけでフォント追加なし」という代替案も
   提示したが、こちらを選択
2. 元ファイルを直すツールは**作らない**
3. 文字コード判定は**自動判定のみ**。`config.ini`への設定項目追加は
   見送った

### 実装

判定・変換とグリフ描画を疎結合な2層に分けた。

**取り込み境界での正規化(`src/text.c`, 新規)**: `text_dup_utf8()`が
判定順「①全バイトASCII→そのまま ②妥当なUTF-8→そのまま
③妥当なCP932→UTF-8変換 ④どちらでもない→ASCIIは通し'?'へ丸め」で
文字列をUTF-8へ正規化する。②を③より先に試すのが肝で、日本語UTF-8
（例: `E3 81 82`）はCP932としても妥当なバイト列になり得るため、順序を
逆にすると誤判定する。CP932→Unicode変換表は`tools/make_cp932_table.py`
（Python標準の`cp932`コーデックのみ使用、外部依存なし）で
`vendor/cp932/cp932_to_ucs.h`へ事前生成してコミットしてある
（ビルド時にPython不要。`tools/make_glyph.py`と同じ方針）。
`src/playlist.c`の`pl_scan_source()`が`gme_track_info()`の
`song`/`game`/`author`/`copyright`/`system`を複製する箇所（`dup_str()`
呼び出し5箇所）を、`dup_meta()`（`text_dup_utf8()`をラップし、NULL時は
空文字列にフォールバックする薄いヘルパ）へ差し替えるだけで済んだ。
拡張M3Uの曲名も`gme_load_m3u_data()`の結果が同じ`info->song`経路を
通るため、追加コードなしで対象になる。

**美咲フォントアトラス(`src/ui.c`)**: `vendor/misaki/misaki_gothic.h`
（美咲フォントBDF版から`tools/make_misaki_font.py`で生成。7076グリフ、
約470KBのCソース）をASCIIとは別の第2テクスチャアトラス
(`ui->cjk_atlas`, 1024x448px)へ展開する`build_cjk_atlas()`を追加した。
`ui_text()`の描画ループで、コードポイントが128未満なら従来の
`font_atlas`、それ以外は`misaki_find()`（`misaki_glyphs[]`がコード
ポイント昇順であることを利用した二分探索）で`cjk_atlas`を引き、
どちらにも無ければ`?`にフォールバックする。**全角も1セル8px幅のまま
描く**のが設計上の要点で、美咲フォントの全角グリフがちょうど8x8
ドットに収まる(FONTBOUNDINGBOX 8 8 0 -2)ため、`ui_text_width()`他の
等幅前提（コードポイント数×グリフ幅）を一切変更せずに済んだ。
`build_cjk_atlas()`が失敗しても`ui_init()`自体は失敗させず、
`cjk_atlas`をNULLのままにして`?`フォールバックへ委ねる（メモリの
厳しい環境で日本語メタデータが理由でアプリごと起動しなくなる事態を
避けるため）。

### BDFからのグリフ変換で気づいた点

美咲ゴシックBDFの`CHARSET_REGISTRY`は`ISO10646`、`CHARSET_ENCODING`は
`1`で、各グリフの`ENCODING`値が**そのままUnicodeコードポイント**に
なっていた。JIS X 0208的なコード変換（`euc_jp`/`iso2022_jp`経由）を
着手前は想定していたが、実際には不要だった。BDFのビットマップは
グリフごとの`BBX`(bbw, bbh, bboffx, bboffy)がフォント全体の
`FONTBOUNDINGBOX`(8, 8, 0, -2)を基準にした相対位置なので、

```
canvas_row = (FBBY + FBBH) - bboffy - bbh + r   (r: BITMAPの行番号, 0起点)
```

で8x8キャンバスへ配置した。ビット順は`font8x8_basic.h`に合わせて
「ビット0=左端」（BDFのMSB-first from-the-leftとは逆）に生成側で
揃えてある。'A'/'あ'/'ア'/'国'/半角'ｱ'をASCIIアートへ変換して目視で
検算してから全件変換した（`tools/make_misaki_font.py`のコメント参照）。

### 検証

- `tests/test_text.c`（新規、SDL/libgme非依存）: このIssueの実NSFから
  直接ダンプしたバイト列（曲名・著作権）を埋め込んだ回帰テスト、
  半角カナ、UTF-8素通し（二重変換防止）、壊れたバイト列の`?`
  フォールバック、`text_cp932_to_utf8()`のバッファ境界（UTF-8
  シーケンスを書きかけで打ち切らないこと）を検証。全18ケース、
  `ctest`で緑
- `./build/muchip --list <NSF>`でゲーム名が
  「ダウンタウンスペシャル くにおく」と正しくUTF-8で出ることを確認
  （「くにおく**ん**」の「ん」が無いのは元ファイルの32バイト上限による
  情報欠落で、これは復元できない）
- `--ui-script`+`--screenshot`でPlayer画面を実際に描画し、拡大した
  スクリーンショットでダ行の濁点まで正しく出ていることを目視確認
- ASan/UBSan (`build-asan`)で単体テスト・実NSFの`--list`・
  `--ui-script`のいずれもエラー無し
- `./scripts/build-aarch64.sh && ./scripts/package.sh`でクロスビルド・
  パッケージングが通ることを確認。`.muxapp`は292,080→366,428バイト
  （+74,348バイト、美咲フォント表とCP932変換表の分）
- 実機（muOS）での物理ボタン操作による確認は本セッションでは未実施
  （開発機に実機が接続されていない）。次回実機作業時に
  Browser/Player/TrackListでの日本語表示とスクロール・折り返しの
  目視確認を追加すること

### 実機検証（完了）

上記の「未実施」を受け、後日ユーザーが新規に用意した RG40XX H
（muOS 2601.1_FUNKY_JACARANDA、aarch64、glibc 2.38・SDL2 2.28.5.0で
既存の`sysroot/`とバイナリ互換であることを事前にバージョン照合して確認済み。
再取得は不要だった）で検証した。

手順:

1. SSH越しに`muChip-1.7.0.muxapp`を`/mnt/mmc/ARCHIVE/`へ転送し、
   `/opt/muos/script/mux/extract.sh`（Archive Managerが内部で呼ぶのと
   同一スクリプト）で展開
2. 問題のNSFファイル自体を`/mnt/mmc/ROMS/GBS/`へ転送
   （`mux_launch.sh`の音楽フォルダ自動検出候補の1つ）
3. `mux_launch.sh`経由の正式起動はSSH越しだと`muxfrontend`のフォアグラウンド
   受け渡しを経由せず画面が固まる既知の問題（Issue #13の実機検証で確認済み）
   があるため今回は行わず、Issue #2の実機検証と同じ`--ui-script`+
   `--screenshot`（`bin/muchip`を直接、`XDG_RUNTIME_DIR=/run`を設定して
   実行）で3画面を撮影した。この経路は実際のディスプレイ・GPU・
   オーディオデバイスを使う（`--window`指定なしなのでSDLは実機の
   実解像度640x480をfullscreenで検出する）

結果:

- **Player**: ログに`再生: [1/18] "Track 01"`が出て実際に音声再生を開始し、
  波形ビジュアライザが静止画ではなく実際の波形を描いていた（無音でない
  間接証拠）。ゲーム名「ダウンタウンスペシャル くにおく」・著作権
  「1991/07/26 テクノスジャパン」とも文字化けせず表示され、拡大した
  スクリーンショットでダの濁点も正しく確認できた
- **Browser**: `/mnt/mmc/ROMS/GBS`配下にNSFファイル1件が正しく列挙された
- **TrackList**: `Tracks (18)`〜`18. Track 18`まで過不足なく列挙された
  （個別トラック名はこのNSFに無いため`Track NN`のフォールバックが
  正しく機能している。ホスト側の`--list`結果と一致）
- ログに`SDL_CreateTexture`失敗や警告は無く、美咲フォントアトラス
  （約1.8MB）の確保も実機のGPU上で問題無かった。3回の起動いずれも
  `config.ini`の保存まで終えて終了コード0

テスト用の一時ファイル（`/tmp/*.bmp`等）はSSH越しに削除済み。
`muChip-1.7.0.muxapp`（`/mnt/mmc/ARCHIVE/`）とテスト用NSF
（`/mnt/mmc/ROMS/GBS/`）は実機に残置した。

### スコープ外（意図的に含めなかったもの）

- **Browserのファイル名表示**: `src/browser.c`の`name`はファイル
  アクセスにも使う文字列で、変換するとファイルが開けなくなる。表示名を
  別途持つ改修が必要で独立した変更として切り出すべき
- **元ファイル（NSF）の書き換えツール**: ユーザー確認済みで作らない。
  他プレーヤーでの文字化けは残る
- **曲名の「ん」の復元**: 元ファイルの32バイト上限で既に失われており
  復元不能
