...
- NSF/GBSヘッダのゲーム名等（固定32バイト）に書かれたShift_JIS(CP932)の
  日本語が、フィールド長で2バイト文字の途中まで切れているファイルで、
  末尾1バイトのために手前の正しく読める部分まで丸ごと文字化けしていたのを
  修正した（Issue #40）。
...
- 画面の配色を切り替えられるようにした（Issue #27）。Settings画面の
  ...
  `Theme` の次の `Edit theme` から開く Theme Editor サブ画面で、9つの
  色スロット（背景・パネル・本文・副文・アクセント・選択行・再生中
  マーク・警告・充電中）をL1/R1でのチャンネル切替とLEFT/RIGHTでの値
  増減により画面から編集でき、変更は即座に4画面へライブプレビューされる。
  `custom` のパレットは `config.ini` の `[theme]` セクション（9キー、
  16進カラー）を手編集することでも変更できる。既存の `config.ini` は
  そのまま読め、`theme` を指定しなければ既定（`midnight`）になるため
  見た目は変わらない

### ライセンス

- muChip 本体を MIT ライセンスとして公開した（Issue #5、`LICENSE`）。
  同梱・静的リンクしている libgme（LGPL-2.1）ほかサードパーティ製
  ソフトウェアのライセンス一覧を `THIRD-PARTY.md` にまとめ、`.muxapp`
  にもライセンス表記一式を同梱するようにした
